<template>
  <div class="taskList">
    ...
    <div class="button-wrapper" v-if="!editedTask">
      <input class="input-tarea" v-model="newTask" type="text" placeholder="Agregar nueva tarea" />
      <button @click="addTask">Agregar</button>
      <button class="delete" @click="clearTasks">Vaciar lista</button>
    </div>

    <div class="button-wrapper" v-else>
      <input class="input-tarea" v-model="newTask" type="text" placeholder="Editar tarea" />
      <button @click="updateTask">Guardar</button>
      <button class="cancel" @click="editedTask = null; newTask = ''">Cancelar</button>
    </div>
  </div>
</template>
...
.taskList .button-wrapper button.delete, .taskList .button-wrapper button.cancel {
  background-color: #f44336;
}
</style>